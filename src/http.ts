/**
 * HTTP layer for the HaloPSA API
 */
...
/**
 * HTTP client for making authenticated requests to the HaloPSA API
 */
export class HttpClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // Build the URL
    let url = `${this.config.baseUrl}/api${path}`;
    if (params) {
  ...
  private async handleResponse<T>(
    ...
    switch (response.status) {
      ...
      case 403:
        throw new HaloPsaForbiddenError('Access forbidden - insufficient permissions', responseBody);

      case 404:
        throw new HaloPsaNotFoundError('Resource not found', responseBody);
      ...
      case 429:
        ...
        throw new HaloPsaRateLimitError(
      ...
      default:
        if (response.status >= 500) {
          ...
          throw new HaloPsaServerError(
            `Server error: ${response.status} ${response.statusText}`,
            response.status,
            responseBody
          );
        }
        throw new HaloPsaError(
  ...
  private isValidationError(responseBody: unknown): boolean {
    if (typeof responseBody === 'object' && responseBody !== null) {
      ...
      // HaloPSA validation errors typically have an errors array or validation_errors
      return Array.isArray(body['errors']) || Array.isArray(body['validation_errors']);
    }
    return false;
  }
  ...
  private parseValidationErrors(responseBody: unknown): Array<{ field: string; message: string }> {
    if (typeof responseBody === 'object' && responseBody !== null) {
      const body = responseBody as Record<string, unknown>;
      const errors = (body['errors'] ?? body['validation_errors']) as unknown;
