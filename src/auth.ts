/**
 * OAuth 2.0 Client Credentials token management for HaloPSA API
 */
...
/**
 * Manages OAuth 2.0 Client Credentials token lifecycle for the HaloPSA API
 */
export class AuthManager {
  ...
  private async doAcquireToken(): Promise<TokenInfo> {
    ...
    // Add tenant ID if provided (for multi-tenant applications)
    if (this.config.tenantId) {
      bodyParams.append('tenant', this.config.tenantId);
    }
    ...
    try {
      ...
      if (error instanceof HaloPsaAuthenticationError) {
        throw error;
      }
      throw new HaloPsaAuthenticationError(
